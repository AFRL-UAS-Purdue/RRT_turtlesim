...
from rrt_python.rrt_test import getEndpos, RRT, dijkstra, simplify_path

class MyPythonNode(Node):
    def __init__(self):
        super().__init__("rrt_python")
        self.get_logger().info("This node attempts to control the turtle using RRT")
        ...
        self.rrt_path = []
        self.rrt_path_branch = -1
        self.branch_tol = 0.2

        self.goal_pose_x = 5.5
        self.goal_pose_y = 5.5
        self.goal_pose_z = 0.0

        self.goal_intpose_x = 5.5
        self.goal_intpose_y = 5.5
        self.goal_intpose_z = 0.0
    ...
    def goal_pose_callback(self, data):
        ...
        # Generate RRT path to follow
        startpos = (self.turtle_pose_x, self.turtle_pose_y) #Latest drone position
        roverpos = (self.goal_pose_x, self.goal_pose_y) #Latest rover position
        velocity = ([[1], [1]]) #Latest velocity of the rover (global)
        roverRadius = 0.01 #Distance from the rover that the drone should follow from
        n_iter = 200 #Maximum number of RRT iterations
        stepSize = 0.7 #Length of RRT segments
        
        ''' Temporary test obstacles '''
        obstacles = [(1., 1.), (8., 2.), (3.5, 3.5), (2., 4.), (4., 1.), (6.,9.), (2.,9.), (4,2.5), (7,7)]
        radius = 1.0

        endpos = (self.goal_pose_x, self.goal_pose_y)
        ...
        if G.success:
            ...
            print("Desired RRT path: ", path)
            self.rrt_path = path
            self.rrt_path_branch = 0
        else:
            print("Yeah something in RRT broke for some reason.")
    ...
    def turtleControl(self):
        # Define intermediate goal pose from path, if path exists
        if self.rrt_path and self.rrt_path_branch < len(self.rrt_path):
            self.goal_intpose_x = self.rrt_path[self.rrt_path_branch][0]
            self.goal_intpose_y = self.rrt_path[self.rrt_path_branch][1]
            self.goal_intpose_z = 0.0

        # Find direction from turtle to goal
        goal_vec = np.array([self.goal_intpose_x, self.goal_intpose_y])
        turtle_vec = np.array([self.turtle_pose_x, self.turtle_pose_y])
        ...
        # Decide when to move to next branch of the RRT path
        intgoal_dist = np.sqrt((self.turtle_pose_x - self.goal_intpose_x)**2 + (self.turtle_pose_y - self.goal_intpose_y)**2)
        if intgoal_dist <= self.branch_tol and self.rrt_path_branch < len(self.rrt_path):
            self.rrt_path_branch += 1
        # print("branch: ", self.rrt_path_branch)
